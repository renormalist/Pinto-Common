...
=for stopwords cpan cpanm BackPAN MyApp URI
...
The C<dev> argument is the name of the initial stack in the
repository.  We'll discuss stacks in more detail later on.
...
In any repository, there never more than one default stack.  When we
created this repository, we specified the initial stack to be C<dev>
and this stack was marked as the default.  You can also change the
default stack, but we won't go into that here.  See the
L<default|App::Pinto::Command::default> command to learn more about that.

Just remember that C<dev> is the name of the stack that we created
when the repository was first set up.
...
gone onto the C<dev> stack.  You could create an entirely new stack,
but the C<dev> stack already has the prerequisites for My-App, so
we're just going to make a clone using the
L<copy|App::Pinto::Command::copy> command:

  $> pinto -r ~/repo copy dev uri_upgrade
...
The listing should be identical to the C<dev> stack:
...
You'll see that the C<dev> stack still contains version 1.60 of URI,
but the C<uri_upgarde> stack now contains version 1.62:

  dev:          rf URI                1.60 GAAS/URI-1.60.tar.gz
...
My-App, and then proceed to upgrade URI on the C<dev> stack.
...
If you look at the listing for the C<dev> stack again, you'll see
something like this:
...
If all goes well, remove the pin from the C<dev> stack and pull your
...
That should display something like this:
  ...
  ---------------------------------------------------------------------------
  dev@4 | jeff | Thu Sep 20 11:12:18 GMT 2012
